...
def train(dataloader, model, loss_fn, optimizer):
    ...
    for batch, (X, y) in enumerate(dataloader):
        ...
        current_acc = torch.mean((torch.round(pred) == y).float())
        ...
        if batch % 100 == 0:
            ...
            print(f'loss: {loss:>7f}  [{current:>5d}/{size:>5d}] time: {end-start} acc: {current_acc}')
            start = time.time()
...
def valid(dataloader, model, loss_fn, save_filename=None):
    ...
    with torch.no_grad():
        for X, y in dataloader:
            ...
            valid_acc += torch.mean((torch.round(pred) == y).float())

    valid_loss /= num_batches
    valid_acc = 100*valid_acc / num_batches
...
def test(dataloader, model, loss_fn):
    ...
    with torch.no_grad():
        for X, y in dataloader:
            ...
            test_acc += torch.mean((torch.round(pred) == y).float())
